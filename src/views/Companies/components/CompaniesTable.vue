<template>
    <b-table :fields="fields" ref="table" sticky-header :provider='myProvider' class="text-nowrap" show-empty empty-text="No existen compañías registradas aún." responsive>
        <template #table-busy>
        ...
        <template #cell(actions)="data">
            <b-badge v-b-tooltip.hover.top="'Eliminar'" class="cursor-pointer" pill variant='danger' @click="deleteCompany(data.item.id)">
                <i-bi-trash3/>
            </b-badge>
            <b-badge v-b-tooltip.hover.top="'Editar'" class="ms-2 cursor-pointer" pill variant='warning'>
                <i-bi-pencil-square/>
            </b-badge>
      </template>
    </b-table>
...
export default {
    ...
    methods:{
        ...
        async deleteCompany(id_company){
            ...
            try {
                const response = await CompaniesServices.deleteCompany(params)
